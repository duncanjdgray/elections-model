...
class Area:
    ...
    def population(self, population):
        if population >= 0:
            self._population = int(round(population))
        else:
    ...
    def children(self, children):
        if isinstance(children, Area) or children == [] or \
           all(isinstance(x, Area) for x in children):
            ...
            raise TypeError(
                "children must only be one or more Area-type objects!")
    ...
    def parties(self, parties):
        if all(isinstance(x, Party) for x in parties):
            ...
            raise TypeError(
                "all elements of parties must be Party-type objects!")
    ...
    def create_voters(self, parties, voteshares):
        ...
        for i in range(0, round(self.population * self.turnout)):
            ...
            # set arguments out of bounds so while loops run at least once
            rnd_lib_auth, rnd_left_right, rnd_rem_leave = -11, -11, -11
            while rnd_lib_auth < -10 or rnd_lib_auth > 10:
                rnd_lib_auth = np.random.normal(
                    loc=rnd_party[0].lib_auth, scale=rnd_party[0].scale_la)
            while rnd_left_right < -10 or rnd_left_right > 10:
                rnd_left_right = np.random.normal(
                    loc=rnd_party[0].left_right, scale=rnd_party[0].scale_lr)
            while rnd_rem_leave < -10 or rnd_rem_leave > 10:
                rnd_rem_leave = np.random.normal(
                    loc=rnd_party[0].rem_leave, scale=rnd_party[0].scale_rl)
            self.voters.append(Voter(name=str(i), 
                                     location=self, 
                                     lib_auth=rnd_lib_auth, 
                                     left_right=rnd_left_right, 
                                     rem_leave=rnd_rem_leave, 
                                     priority_axis=rnd.choices([0, 1, 2, 3], 
                                                               [10, 1, 2, 2], 
                                                               k=1)[0], 
                                     tactical=rnd.choices([True, False], 
                                                          [tactical_voting_factor, 
                                                          1 - tactical_voting_factor])[0]))  
            # most voters care about all axes, some prioritise one, but fewer prioritise lib-auth than left-right or rem-leave
    ...
    def tally_votes(self, system):
        ...
        if system == "FPTP":
            ...
            self._votes_sorted = sorted(
                self.votes, reverse=True, key=lambda tup: tup[1])
        else:
            raise ValueError(
                "system not set to recognised value. Recognised values are: 'FPTP'")
    ...
    def declare_winner(self, system):
        ...
        if system == "FPTP":
            print("The winner in " +
                  ...
                  self.winners.name +
                  " with " +
                  str(self._votes_sorted[0][1]) +
                  " votes, beating the next closest party " +
                  self._votes_sorted[1][0].name +
                  " which got " +
                  str(self._votes_sorted[1][1]) +
                  " votes.")
        else:
            raise ValueError(
                "system not set to recognised value. Recognised values are: 'FPTP'")
    ...
    def call_election_fptp(self, declare_winners=False):
        for child in self.children:
            print("Running elections for: " + child[0].name)
            child[0].call_election_fptp()
            for party in child[0].winners:
                self.winners.append(party)
        # unpack list of lists to be flat list
        self.winners = list(np.asarray(self.winners, dtype=object).flatten())
        self.winners_count = [(x, self.winners.count(x))
                              for x in set(self.winners)]
        self._winners_count_sorted = sorted(
            self.winners_count, reverse=True, key=lambda tup: tup[1])
        if declare_winners:
            print("The result of the election in " + self.name + " is:\n")
            print(*self._winners_count_sorted, sep='\n')
            if int(
                    self._winners_count_sorted[0][1]) > sum(
                    x for _,
                    x in self._winners_count_sorted) / 2:
                print(self._winners_count_sorted[0][0].name +
                      " has won an overall majority with " +
                      str(self._winners_count_sorted[0][1]) +
                      " out of " +
                      str(sum(x for _, x in self._winners_count_sorted)) +
                      " total seats.")
            else:
    ...
    def get_local_votes_from_children(self):
        if self.children != []:
            ...
            for party in self.parties:
                self.local_voteshares[party] = self.local_votecounts[party] / \
                    sum(self.local_votecounts.values())

        if self.local_voteshares == {}:
            for party in self.parties:
                self.local_voteshares[party] = party.voteshare
                self.local_votecounts[party] = self.local_voteshares[party] * \
                    self.population
...
class Constituency(Area):
    ...
    def call_election_fptp(self, declare_winners=False):
        self.create_voters(
            list(
                self.local_voteshares.keys()), list(
                self.local_voteshares.values()))
...
class Actor:
    ...
    def __str__(self):
        return """Name: {name}, Lib-Auth: {lib_auth}, Left-Right: {left_right}, Remain-Leave: {rem_leave}""".format(
            name=self.name, lib_auth=self.lib_auth, left_right=self.left_right, rem_leave=self.rem_leave)
    ...
    def left_right(self, left_right):
        if left_right >= -10 and left_right <= 10:
            ...
            raise ValueError(
                "left_right must be between -10 and 10 inclusive!")
...
class Voter(Actor):
    ...
    def priority_axis(self, priority_axis):
        if priority_axis in map_priority_axes:
            ...
            raise ValueError(
                "priority_axis can only have values from " +
                map_priority_axes.keys())
    ...
    def order_parties(self):
        self.ordered_parties = sorted(
            self.parties_dist, key=lambda tup: tup[1])
    ...
    def vote(self, system):
        if system == "FPTP":
            ...
            if self.tactical:
                tactical_parties = pd.DataFrame.from_dict(
                    self.location.local_voteshares, orient='index')
                tactical_parties.reset_index(level=0, inplace=True)
                tactical_parties.sort_values(
                    by=[0], ascending=False, inplace=True)
                tactical_parties.drop(
                    index=tactical_parties.index[2:], axis=0, inplace=True)
                voting_order = [
                    i for i in self.ordered_parties if (
                        i[0] in tactical_parties['index'].values)]
