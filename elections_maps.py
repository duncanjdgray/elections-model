...
ward_la_country = pd.DataFrame(pd.read_csv("data/map_ward_la_county_region_country.csv")) \
    .apply(lambda x: x.astype(str).str.lower()) \
    .apply(lambda x: x.astype(str).str.replace("ô", "o"))       # 'ynys môn' causes problems with matching
...
map_la_country = pd.Series(ward_la_country['CTRY18NM'].values, index=ward_la_country['LAD18NM']).to_dict()
...
list_country = ward_la_country_lists[10]
